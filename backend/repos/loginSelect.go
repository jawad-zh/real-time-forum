...
func LoginSelect(user *models.Login) (bool, *models.Users,string) {
	...
	SELECT 	UserID , Nickname , Age , Gender , FirstName , LastName , Email , Password , CreatedAt FROM Users
	WHERE Email = ? OR Nickname = ?
	`,user.NicknameOrEmailInput , user.NicknameOrEmailInput)
	err:=row.Scan(&UserInfo.UserID , &UserInfo.Nickname,
	&UserInfo.Age,&UserInfo.Gender,&UserInfo.FirstName,&UserInfo.LastName,&UserInfo.Email,&UserInfo.Password,&UserInfo.CreatedAt)
